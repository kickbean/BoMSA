...
{\bf Vanishing gradients:} Vanishing gradients~\cite{bengio1994learning} refers to the phenomena where gradients magnitudes decrease as they are propogated. This makes CNN training difficult, particularly for low-layers. In our DAG-CNN model, lower layers are directly connected to the output layer through multiscale connections, ensuring they recieve a strong signal during learning. Fig.~\ref{fig:grad} experimentally verifies this claim.% DAG-CNN at the bottom layer is consistently larger than that of a chain model during training time. This make multi-scale training considerably more efficient than single (e.g., high-level) scale training.
...
\caption{The average gradient from Layer-1 (Conv) during training, ploted in log-scale. Gradients from the DAG are consistently 10X larger, implying that they receive a stronger supervised signal from the target label during gradient-based learning.}
\label{fig:grad}
\end{figure}
