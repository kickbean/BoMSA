...
\begin{figure*}[htbp]
\centering
	\subfigure[low-level feature is preferred]{\includegraphics[width=.9\textwidth]{moti_low_better.png}\label{fig:moti_low}}
	\subfigure[high-level feature is preferred]{\includegraphics[width=.9\textwidth]{moti_high_better.png}\label{fig:moti_high}}
...
\begin{figure}[htbp]
\centering
	\subfigure[Caffe]{\includegraphics[width=.43\columnwidth]{fig_add_back_caffe.png}\label{fig:add_back_caffe}}
	\subfigure[Deep19]{\includegraphics[width=.56\columnwidth]{fig_add_back_deep19.png}\label{fig:add_back_deep19}}
...
\begin{figure}[htbp]
\centering
	\includegraphics[width=\columnwidth]{fig_model.png}
...
\begin{figure}[htbp]
\centering
	\includegraphics[width=\columnwidth]{fig_backprop_eq.png}
\caption{Visualization of parameter setups at $i$-th ReLU.}
...
\begin{enumerate}
\item \cite{Gong14} computes multiple feed-forward on a single image due to multiple patches. The computation of convolution is the bottleneck for this procedure. In our approach, our model is trained to adapt to multi-scale features. Thus, during test phase,
only one feed-forward computation is needed for one image. As matter of fact, extracting feature of one image in~\cite{Gong14} takes more than 20 seconds while ours only take less than 1 second. 
...
In this section, we carried out experiments on benchmark dataset, SUN397~\cite{SUN397}, MIT67~\cite{MIT67}, and Scene15~\cite{Scene15} to demonstrate the consistency of our approach. 
