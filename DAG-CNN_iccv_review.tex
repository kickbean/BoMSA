...
\begin{figure}[t!]
\centering
\subfigure[Avg-pooling]{\includegraphics[width=.49\columnwidth]{fig/sumpool_size.png}\label{fig:sumpool_size}}
\subfigure[Max-pooling]{\includegraphics[width=.49\columnwidth]{fig/maxpool_size.png}\label{fig:maxpool_size}}
\caption{\new{Greedy selection of layers and the number of pooling windows per layer. In general, globally-pooled $1 \times 1$ features (with average pooling)  performs the best.} \deva{Songfan, could you add the result that searches over all window sizes during the greedy selection? You already showed such a result, and could probably just add a line it to the above graphs (labeled something link  [1x1-4x4]). If space is an issue, I'd drop the max-plot.}}
\label{fig:pool_size}
\end{figure}
