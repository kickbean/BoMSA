...
\caption{Deep19-DAG results on MIT67. The category label is shown on the left and the label for false-positives (in {\bf red}) are also provided. We use the multi-scale analysis of Fig.~\ref{fig:level_perf} to compare categories that perform better with mid-level features ({\bf top 2 rows}) versus high-level features ({\bf bottom 2 rows}). Mid-level features appear to emphasize \textit{grid-textured} objects for {\tt trainsition} scenes and \textit{circular } objects for {\tt laundromat}, while high-level features appear to focus on global spatial statistics. \deva{This explanation doesn't match with Fig4. Could we pick one class per layer to visualize? Should transition be train station?}}
\label{fig:more_eg}
\end{figure*}
