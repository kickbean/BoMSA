...
Feature selection on Caffe-DAG selects layers 11,  13, 15, 18, and 20, for a total of 9216 feature dimensions, {\em making it only a factor of 2X larger} and so rather easy to use in practice. Similar, feature selection on Deep19-DAG selects layers \deva{....}.
% multi-scale model generates a feature vector of size 9216, {\em making it only a factor of 2 larger} and so rather easy to use in practice. We provide the exact breakdown of feature dimensionality for our Caffe-DAG model in Table.~\ref{table:feat_dim}.
%\begin{table}[t!]
%\begin{center}
...
%\end{tabular}
%\end{center}
%\caption{Feature Dimension for Caffe-DAG. For reference, we also show the dimensionality obtained using all activations from a layer, rather than just the marginal ones. Marginal features reduce dimensionality by nearly 20X, while preserving accuracy (as shown in Fig.~\ref{fig:full_marg}). }%\deva{We should also include the equivalent for Deep19. Because we likey don't have space, I think its fine to include this as text of the form ``Feature selection on Caffe-DAG selects Layers 11,  13, 15, 18, and 20, for a total of 9216 features, only a factor of 2X larger. Similar, feature selection on Deep19-DAG selects Layers x,x,x,....''}}
