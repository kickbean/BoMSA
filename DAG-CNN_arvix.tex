...
We use DAG-CNNs to learn multiscale features that can be shared between coarse and fine-grained classification tasks. Indeed, we show that multiscale features transfer extraordinarily well between classification tasks, reducing the need for fine-tuning. We present extensive analysis and results on standard scene benchmarks and show state-of-the-art classification performance on all benchmarks, \ie 55.5\% on SUN397, 76.1\% on MIT67, and 92.4\% on Scene15.
