...
\caption{Histogram of the best-performing layer. Interestingly, the last layer ({\bf 20}) is optimal for only 15 classes, while the second-to-last layer ({\bf 18}) proves most discriminative for 26 classes. Finally, mid-level layers ({\bf 11,13,15}) are most effective for the remaining 26 classes. Indeed, the third-most effective layer is approaching a mid-low level feature ({\bf 11}). These results validate our underlying hypothesis; different classes require different amounts of invariance. This suggests that a feature extractor shared across such classes will be more effective when multi-scale.}
...
{\bf Marginal features:} In the next section, we will explore multi-scale features. One practical hurdle to including all features from all layers is the massive increase in dimensionality. Here, we explore strategies for reducing dimensionality through pooled features. We consider various pooling (sum, average, and max) and  normalization (with and without L2 normalization) strategies. We saw good results with average pooling over all spatial locations, followed by L2 normalization. Specifically, assume a particular layer is of size $H \times W \times F$, where $H$ is the height, $W$ is the width, and $F$ is the number of filter channels. We compute a $1 \times 1 \times F$ feature by averaging across spatial dimensions. %Note that the ``marginal activations'' of fully-connected layers (which are already of dimension $1 \times 1 \times F$) are simply the original activations.  
We compare the performance of using the original versus marginal feature in Fig.~\ref{fig:full_marg}. Marginal features always do better, implying dimensionality reduction actually helps performance. We verified that this phenomena was due to over-fitting; the full features always performed better on training data, but performed worse on test data. This suggests that with additional training data, full-dimensionality features may perform better.
...
{\bf Feature dimensionality:} Most existing methods that use CNNs as feature extractors work with the last layer (or the last fully connected layer), yielding a feature vector of size 4096. Our multi-scale model generates a feature vector of size 9216, {\em making it a factor of 2 larger} and so rather easy to use in practice. We provide the exact breakdown of feature dimensionality for our Caffe-DAG model. The five selected layers and their associated (marginal) dimensionality is given in Table.~\ref{table:feat_dim}.
\begin{table}[htbp]
\begin{center}
...
{\bf DAG:} Importantly, we explore an ``off-the-shelf'' version of our DAG, DAG-OTS. This model is obtained by extracting multi-scale features from a pre-trained CNN. We stress that this feedforward model is still a DAG. It just so happens that this DAG is rather easy to train, as one can pass these multi-scale features to a standard linear classification package (e.g., SVM). We compare this model to a fine-tuned vesion that is trained using a target dataset, making use of the modified backprop equation from Sec.~\ref{sec:approach}. %We will show that off-the-shelf DAGs perform quite well, though fine-tuning does further improve performance.

Fig.~\ref{fig:comp_otf} reports the comparison of off-the-shelf and fine-tune features on both Chain and DAG models. Significant improvements of DAG over Chain are observed in all three datasets, suggesting that the multi-scale features indeed plays an essential role for scene classification. Further, fine-tuned features out-perform the off-the-shelf features consistently. This shows that the data specific information is well captured by fine-tuning, which further boosts the recognition performance. Perhaps most impressive is the strong performance of DAG-OTS. From a theoretical perspective, this validates our underyling hypothesis that multi-scale features allow for better transfer between recognition tasks -- in this class, ImageNet and scene classification. An interesting question is if multi-scale features trained on ImageNet will allow for even more transfer. We are currently exploring this. From a practical perspective, our results suggest that {\em any system making use of off-the-shelf CNN features should explore multi-scale variants as a ``cheap" and ``easy-to-implement'' baseline.} 
...
\begin{figure}[t]
\centering
	\subfigure[SUN397]{\includegraphics[width=.32\columnwidth]{fig/comp_sun.png}\label{fig:comp_sun}}
	\subfigure[MIT67]{\includegraphics[width=.32\columnwidth]{fig/comp_mit.png}\label{fig:comp_mit}}
	\subfigure[Scene15]{\includegraphics[width=.32\columnwidth]{fig/comp_scene.png}\label{fig:comp_scene}}
