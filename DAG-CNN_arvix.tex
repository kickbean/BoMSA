...
\begin{figure}[htbp]
\centering
	{\includegraphics[width=.8\columnwidth]{fig/full_marg_all.png}}

\caption{Single-scale classification accuracy (\%) of Full vs. Marginal Features.\songfan{It seems that including the comparison of ALL layers does not support the statement `` dimensionality reduction actually helps performance" }}
\label{fig:full_marg}
\end{figure}
...
\hline
SUN397  & 43.5 & ? & 46.6 & 47.5 \\
MIT67 & 59.5 & 61.7  & 64.6 & 65.4 \\
Scene15 & 86.8 & ? & 89.7 & 91.1 \\
\hline
\end{tabular}
\end{center}
\caption{Comparison of chain, off-the-shelf (OTS) , and fine-tuned (FT) models using the Caffe backbone. \deva{Two requests. Can you replace DAG-FT with the result that retrains a SVM classifier using DAG features? I think its fine to still call this a DAG-FT, one that just uses a batch optmiztion instead of SGD. Hopefully we can increase the 64.2 number a bit. Can you fill in the (?) above? If so, then I think that this table (perhaps better shown as a bar graph, similar to Fig 12) would have a nice diagnostic analysis of fine-tuning, off-the-shelf, and single vs multi-scale.} \songfan{if I use SVM for the DAG-FT features, the results for SUN397 and MIT67 improves even further; but not for the Scene15. So I left the 91.1 as is since it is still a DAG-FT result using SGD. I will complete experiment for the other two numbers tomorrow and then turn this table to a bar chart. Given these results, I assume we can get rid of the DAG-Deep19 table, right?}}
